...
def readParams(fichajes: List[str]):
    ...
    tiempo_restante_ahora = tiempo_trabajo_base - tiempo_trabajado_ahora
    if tiempo_restante_ahora < timedelta():
        tiempo_restante_ahora = timedelta()
    ...
    hora_rest_ahora, minuto_rest_ahora, _ = convert_timedelta(tiempo_restante_ahora)
    ...
    if tiempo_trabajado > tiempo_trabajo_base:
        ...
        return {
            ...
            "Tiempo usado en descansos": f"{formatTo2Digits(hora_des)}:{formatTo2Digits(minuto_des)}:00",
            "Tiempo trabajado hasta último fichaje": str(tiempo_trabajado).zfill(8),
            "Tiempo restante desde último fichaje": "00:00:00",
            "Tiempo trabajado hasta ahora": f"{formatTo2Digits(hora_ahora)}:{formatTo2Digits(minuto_ahora)}:00",
            "Tiempo restante hasta ahora": f"{formatTo2Digits(hora_rest_ahora)}:{formatTo2Digits(minuto_rest_ahora)}:00",
        ...
        return {
            ...
            "Tiempo usado en descansos": f"{formatTo2Digits(hora_des)}:{formatTo2Digits(minuto_des)}:00",
            "Tiempo trabajado hasta último fichaje": str(tiempo_trabajado).zfill(8),
            "Tiempo restante hasta último fichaje": f"{formatTo2Digits(hora_res)}:{formatTo2Digits(minuto_res)}:00",
            "Tiempo trabajado hasta ahora": f"{formatTo2Digits(hora_ahora)}:{formatTo2Digits(minuto_ahora)}:00",
            "Tiempo restante hasta ahora": f"{formatTo2Digits(hora_rest_ahora)}:{formatTo2Digits(minuto_rest_ahora)}:00",
            "Hora de salida estimada": f"{formatTo2Digits(hora_salida)}:{formatTo2Digits(minuto_salida)}:00"
        }
